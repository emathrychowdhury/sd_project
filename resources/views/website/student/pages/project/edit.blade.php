...
<div class="container">
        <h3>Edit Project</h3>
        <form method="post" action="{{URL::to('update-project/'.$project->id) }}">
        {{ csrf_field() }}
		...
		@endif
            <div class="form-group">
                ...
                <select name="course_id" class="form-control" id="">
                    ...
                    @foreach ($courses as $c)
                        <option value="{{ $c->id }}"{{$project->course_id == $c->id ? 'selected' : ''}}>{{ $c->title }}</option>
                    @endforeach
                </select>
            </div>
            <!-- <div class="form-group">
            ...
            <div class="form-group  my-3">
                ...
                <input type="text" name="title" value="{{$project->title}}" class="form-control" id="">
            </div>
            <div class="form-group my-3">
            ...
            @foreach($members as $m)
                <div class="col-sm-6 m-0 mb-2 d-flex">
                    <select name="members[]" class="form-control" required>
                        ...
                        <option value="{{$user->studentid}}"{{$m == $user->studentid ? 'selected' : ''}}>{{$user->studentid}}</option>
                        @endforeach
                    </select>                 
